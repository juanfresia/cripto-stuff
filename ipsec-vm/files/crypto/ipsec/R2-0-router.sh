...
#----------------------------------------------------------------------------------------------------
# Scripts: R2-0-router.sh 
# Descripcion: 	El objetivo de este script es configurar el Router2 de manera interactiva
# Parametros:Ninguno
#
...
echo ">>> Script de configuracion interactiva de R2"
Pausa
...
Pausa
echo ">>> Ejecutando: R2-1-preparar.sh"
./R2-1-preparar.sh
if [ $? -ne 0 ]; then
  exit $?
fi
...
Pausa
echo ">>> Ejecutando R2-2-generarclaves.sh"
./R2-2-generarclaves.sh
Pausa
...
Pausa
echo ">>> Ejecutando R2-3-obtenerclaveremota.sh"
./R2-3-obtenerclaveremota.sh
COPIADA=$?
while [ $COPIADA -ne 0 ]
do
  Pausa
